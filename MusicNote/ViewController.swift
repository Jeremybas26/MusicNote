...
class StatsViewController: UITableViewController {
    ...
    // 1. Insert a Section enum and intervals/ivTrainer
    private let ivTrainer = IntervalTrainer.shared
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        navigationItem.leftBarButtonItem = UIBarButtonItem(
            title: "Back", style: .plain, target: self, action: #selector(close))
        navigationItem.rightBarButtonItem = UIBarButtonItem(
            title: "Graph", style: .plain, target: self, action: #selector(openGraph))
        title = "Your Stats"
        tableView.register(UITableViewCell.self, forCellReuseIdentifier: "cell")
        letters = trainer.activeLetters().sorted()
    }

    // 2. Update numberOfSections
    override func numberOfSections(in tableView: UITableView) -> Int {
        Section.allCases.count
    }

    // 3. Update tableView(_:numberOfRowsInSection:)
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let sec = Section(rawValue: section) else { return 0 }
        switch sec {
        case .notes:      return letters.count
        case .intervals:  return intervals.count
        }
    }

    // 4. Update cellForRowAt
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .notes:
            ...
            let s = trainer.stat(for: letter)
            let acc = s.total == 0 ? 0 : Int((Double(s.correct)/Double(s.total))*100)
            let avg = String(format: "%.2f", s.avgSpeed)
            let top = String(format: "%.2f", s.topSpeed)
            cell.textLabel?.text = "\(letter): \(s.correct)/\(s.total)  \(acc)%  • Avg \(avg) n/s  • Top \(top) n/s"

        case .intervals:
            let ivl = intervals[indexPath.row]
            let st  = ivTrainer.stat(for: ivl)
            let acc = st.attempts == 0 ? "—" :
                String(format: "%.0f%%", Double(st.correct)/Double(st.attempts)*100)
            let speed = st.bestSpeed == .greatestFiniteMagnitude ? "—" :
                String(format: "%.2f s", st.bestSpeed)
            cell.textLabel?.text = "\(ivl.rawValue): \(st.correct)/\(st.attempts)  \(acc)  • Best \(speed)"
        }
        return cell
    }
